...
export interface SchemaSelectorProps extends ResourceSelectorProps {
  db: DB;
...
export const SchemaSelector = ({ catalog, schema, db, dialect, onChange, preconfiguredSchema, unityCatalogEnabled }: SchemaSelectorProps) => {
  ...
  */
  ...
  const hasPreconfigCondition = !!preconfiguredSchema || dialect === 'postgres';
  ...
  useEffect(() => {
      ...
      if (!schema) {
        ...
        if (state.value && state.value.find((v) => v.value === schema) === undefined) {
